...
networkRecall <- function(filePath, allDatabases, individualDatabases){
  ...
  for (iPathway in 1:nrow(thisPathway)) {
    ...
    databaseIndexes <-  strsplit(x = directedMatchRxns$INTERACTION_DATA_SOURCE, ";") %>%
      unlist() %>%
      sort() %>%
      unique() %>%
      match(individualDatabases) * 2
    databaseIndexes <- sort(c(databaseIndexes)) + 
      length(pathwayNames)
    ...
    databaseIndexes <-  strsplit(x = undirectedMatchRxns$INTERACTION_DATA_SOURCE, ";") %>%
      unlist() %>%
      sort() %>%
      unique %>%
